...
class TestRedisCommands(object):
    ...
    def test_slowlog_get(self, r, slowlog, ns):
        ...
        get_command = b' '.join((b'GET', (ns + unicode_string).encode('utf-8')))
    ...
    def test_slowlog_get_limit(self, r, slowlog, bns):
        assert r.slowlog_reset()
        r.get('foo')
        r.get('bar')
        ...
        assert isinstance(slowlog, list)
        commands = [log['command'] for log in slowlog]
        assert b'GET %sfoo' % bns not in commands
        assert b'GET %sbar' % bns in commands
    ...
    def test_randomkey(self, r):
        ...
        for key in ('a', 'b', 'c'):
            r[key] = 1
        assert r.randomkey() in (with_bns(b'a'), with_bns(b'b'), with_bns(b'c'))
